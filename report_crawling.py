...
def parse_args():
    ...
    parser.add_argument("sdate", help="검색기간 시작 날짜 (YYYY-MM-DD 형태로 입력)")
    parser.add_argument("edate", help="검색기간 끝 날짜 (YYYY-MM-DD 형태로 입력)")
    parser.add_argument("--analyst", default="", help="검색할 ANALYST 명 입력")
...
def main():
    ...
    driver.get(HKurl(args.sdate, args.edate)) # 오류출력 고치기

    # ANALYST 검색
    if args.analyst!='':
        search_element = driver.find_element(By.ID, 'search_text')   # 검색창
        search_element.send_keys(args.analyst)   # 검색어 입력
    ...
    while True:
        ...
        # next page
        page +=1
        url = HKurl(args.sdate, args.edate, page=page, analyst_quoted=(analyst_quoted if args.analyst!="" else ""))    # 오류 출력되는 부분
        driver.get(url)
    ...
    df.to_csv(f'consensus_{args.analyst}_{args.sdate.replace("-",".")}~{args.edate.replace("-",".")}.csv', encoding='utf-8', index=False)
    print('\nSAVE COMPLETE')
